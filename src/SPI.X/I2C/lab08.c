...
void main(void) {
    ...
    MCP23008_INIT();
    // config CTRL_REG 1
    I2C1_WriteRegister(0x18,0x20, 0b01000111);
    // config CTRL_REG 4 to use block update
    I2C1_WriteRegister(0x18, 0x23, 0b10000000);
    // set range to +/- 2g range, done above^^

    uint8_t who = I2C1_ReadRegister(0x18,0x0F);
...
//----------------------------------------------------------------------------
//Precondition: nothing
//Postcondition: values at GPI pins
//function: gets values at GPI pins
//----------------------------------------------------------------------------
...
//----------------------------------------------------------------------------
//Precondition: 8 bit data
//Postcondition: nothing
//function: writes to the Pins
//----------------------------------------------------------------------------
...
void GoToMain(){
   ...
   while(leave == 0){
       ...
       TFT_DrawString(40,80,"LAB 8",TFT_YELLOW,TFT_RED,3);
...
void GoToLIS3DH(){
    ...
    while(leave == 0){
        ...
        x = I2C1_ReadRegister(0x18,0x28);
        y = I2C1_ReadRegister(0x18,0x2A);
        z = I2C1_ReadRegister(0x18,0x2C);
        ...
        x |= (( I2C1_ReadRegister(0x18, 0x29)  ))<< 8;
        y |= (( I2C1_ReadRegister(0x18, 0x2B)  ))<< 8;
        z |= (( I2C1_ReadRegister(0x18, 0x2D)  ))<< 8;
